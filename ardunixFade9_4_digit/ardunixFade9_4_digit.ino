...
#define MODE_HOURS_SET                  1 // Mode "00"
#define MODE_MINS_SET                   2 // Mode "01"
#define MODE_SECS_SET                   3 // Mode "02"
#define MODE_DAYS_SET                   4 // Mode "03"
#define MODE_MONTHS_SET                 5 // Mode "04"
#define MODE_YEARS_SET                  6 // Mode "05"

// Basic settings
#define MODE_12_24                      7  // Mode "06" 0 = 24, 1 = 12
#define HOUR_MODE_DEFAULT               false
#define MODE_LEAD_BLANK                 8  // Mode "07" 1 = blanked
#define LEAD_BLANK_DEFAULT              false
#define MODE_SCROLLBACK                 9  // Mode "08" 1 = use scrollback
#define SCROLLBACK_DEFAULT              false
#define MODE_FADE                       10 // Mode "09" 1 = use fade
#define FADE_DEFAULT                    false
#define MODE_DATE_FORMAT                11 // Mode "10"
...
// anode pins
#define ledPin_a_6_old  0     // low  - Secs  units UNUSED // package pin 2  // PD0
#define ledPin_a_5_old  1     //      - Secs  tens  UNUSED // package pin 3  // PD1
...
byte anodePins[4] = {ledPin_a_1, ledPin_a_2, ledPin_a_3, ledPin_a_4};
...
long chimePulseEndMs = 0; // When we want the pulse to end
long nextTimePulseStartMs = 0; // When we want the next pulse to start
#define chimePulseLengthMs 100;
#define chimeLengthBetweenPulsesMs 500;
...
void setup()
{
  ...
  pinMode(ledPin_a_5_old, OUTPUT);
  pinMode(ledPin_a_6_old, OUTPUT);
  digitalWrite(ledPin_a_5_old, LOW);
  digitalWrite(ledPin_a_6_old, LOW);
...
void loop()
{
  ...
  if ((chimePulseEndMs > 0) && (nowMillis > chimePulseEndMs)) {
    ...
    // see if there are any more chimes to do
    if (chimesRemaining > 0) {
      nextTimePulseStartMs = nowMillis + chimeLengthBetweenPulsesMs;
      chimesRemaining--;
    }
  }

  if ((nextTimePulseStartMs > 0) && (nowMillis > nextTimePulseStartMs)) {
    nextTimePulseStartMs = 0;
...
// Break the time into displayable digits
// ************************************************************
void loadNumberArrayYear() {
  NumberArray[3] = (year() / 1) % 10;
  NumberArray[2] = (year() / 10) % 10;
  NumberArray[1] = (year() / 100) % 10;
  NumberArray[0] = (year() / 1000) % 10;
}

// ************************************************************
// Break the temperature into displayable digits
// ************************************************************
void loadNumberArrayTemp() {
...
void loadNumberArrayTestDigits() {
  NumberArray[3] =  second() % 10;
  NumberArray[2] = (second() + 1) % 10;
  NumberArray[1] = (second() + 2) % 10;
  NumberArray[0] = (second() + 3) % 10;
}
...
// ************************************************************
void loadNumberArrayConfInt(int confValue, byte mode) {
  NumberArray[3] = (confValue / 1) % 10;
  NumberArray[2] = (confValue / 10) % 10;
  NumberArray[1] = mode % 10;
  NumberArray[0] = mode / 10;
}

// ************************************************************
// Show a boolean configuration value
// ************************************************************
void loadNumberArrayConfBool(boolean confValue, byte mode) {
  int boolInt;
  if (confValue) {
    boolInt = 1;
  } else {
...
// ************************************************************
// Show the configuration value
// ************************************************************
void loadNumberArrayConfItem(byte val) {
  NumberArray[3] = val % 10;
